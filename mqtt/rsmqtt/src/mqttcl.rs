...
pub fn mqtt_start<F>(handle: F)
where
    F: Fn(messdata::Mess),
{
    let rb_server = "raspi3c:1883";
    let rb_channel = "/sensor/om1";
    // configure logging
    env::set_var(
        "RUST_LOG",
    ...
    thread::spawn(move || {
        ...
        let mut next_ping_time =
            last_ping_time + Duration::from_secs((keep_alive as f32 * 0.9) as u64);
        loop {
    ...
    loop {
        ...
        match packet {
            ...
            VariablePacket::PublishPacket(ref publ) => {
                ...
                debug!("Received ({}): {}", publ.topic_name(), msg);
                let js = json::parse(&msg).unwrap();
                let m = messdata::from_json(js);
                handle(m);
            }
            _ => {}
        }
